  ...
  switch (parsed.tipo) {
    ...
    case 'boleto_parcelado':
      ...
      for (var i = 0; i < n; i++) {
        ...
        resultado.pagamentos.push(_boleto(dados, valor, dias[i], i + 1, n));
      }
      break;
    ...
    case 'composto':
      ...
      if (parsed.restante_tipo === 'boleto' || parsed.restante_tipo === 'boleto_parcelado') {
        ...
        for (var j = 0; j < rn; j++) {
          var rv = (j === rn - 1) ? rvu : rvp;
          resultado.pagamentos.push(_boleto(dados, rv, rdias[j], 'restante_' + (j + 1), rn));
        }
      } else if (parsed.restante_tipo === 'cartao') {
        resultado.pagamentos.push(_cartao(dados, resultado.valor_restante, { bandeira: 'VISA', parcelas: dados.parcelas_restantes || 1, debito: false }));
      } else {
        resultado.pagamentos.push({ parcela: 'restante', tipo: 'manual', forma: 'manual', valor: resultado.valor_restante, observacao: 'Registrar manualmente' });
      }
      break;
    ...
    default:
      ...
      resultado.pagamentos.push({ parcela: 1, tipo: 'manual', forma: parsed.forma || formaPagamento, valor: vt, observacao: 'Pagamento em ' + (parsed.forma || 'manual') + ' - registrar no Odoo' });
      break;
  }
...
function _pix(dados, valor, parcela) {
  ...
  return {
    ...
    pix_copia_cola: '00020126580014br.gov.bcb.pix0136' + txid + '5204000053039865802BR5925EMPRESA TESTE6009SAO PAULO62070503***6304' + Math.floor(Math.random() * 9999),
    valor: valor,
...
function _boleto(dados, valor, dias, parcela, total) {
  ...
  var ld = '34190' + nn + '5' + nnum.substring(0, 5) + ' 4' + nnum.substring(5) + ' ' + venc.replace(/-/g, '').substring(2) + ' 1' + ts.slice(-7) + ' ' + cb.slice(-14);
  return {
